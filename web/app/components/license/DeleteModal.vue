...
async function onSubmit() {
  ...
  try {
    ...
    const response = await $fetch<ApiMessageResponse>('/api/license/delete', {
      method: 'POST',
      body: { ids: props.ids } as LicenseDeleteBody
    })
    ...
    toast.add({ title: '删除成功', description: `已删除 ${props.ids.length} 条许可证`, color: 'success' })
